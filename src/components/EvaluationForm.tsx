...
export const EvaluationForm: React.FC<EvaluationFormProps> = ({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    await onSubmit({
      ratings: { technical, physical, psychological, social },
      coachNotes,
      generatedReport: includeAiReport ? (draftReport || undefined) : undefined,
    });
  };
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800/80">
            <div>
              ...
              <p className="text-slate-400 text-xs mt-1">
                Generate custom recommendations, actionable feedback, and training tips for grassroots players.
              </p>
            </div>
            <div className="flex items-center gap-3 bg-slate-900/60 px-4 py-2.5 rounded-xl border border-slate-800">
              <span className="text-xs font-semibold text-slate-300">Include AI Report</span>
              <button
                type="button"
                onClick={() => {
                  setIncludeAiReport(!includeAiReport);
                  if (includeAiReport) {
                    setDraftReport(null);
                  }
                }}
                className={`w-12 h-6 rounded-full p-0.5 transition-colors duration-200 focus:outline-none cursor-pointer ${
                  includeAiReport ? 'bg-emerald-500' : 'bg-slate-800'
                }`}
              >
                <div
                  className={`bg-white w-5 h-5 rounded-full shadow-md transform transition-transform duration-200 ${
                    includeAiReport ? 'translate-x-6' : 'translate-x-0'
                  }`}
                />
              </button>
            </div>
          ...
          {includeAiReport ? (
            <div className="space-y-5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h4 className="text-sm font-semibold text-slate-200">Generate Report Draft</h4>
                  <p className="text-[11px] text-slate-500 mt-0.5">
                    Click to request Gemini to build the grassroots-tuned review.
                  </p>
                </div>
                <button
                  type="button"
            ...
            <div className="bg-slate-950/40 border border-dashed border-slate-800 rounded-xl py-6 px-4 text-center text-slate-400 text-xs flex flex-col items-center gap-1.5">
              <span className="font-bold text-slate-200">✓ Quick Save Mode Active</span>
              <p className="text-[11px] text-slate-500 max-w-md">
                You are saving score ratings and coach notes only. The system will skip generating a detailed AI report.
              </p>
            </div>
          )}
        </div>
